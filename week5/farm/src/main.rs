...
fn get_a_number(input_numbers : Arc<Mutex<VecDeque<u32>>>) -> Option<u32> {
    let mut  guard = input_numbers.lock().unwrap();
    guard.pop_front()
}
...
fn main() {
    ...
    for i in 0..num_threads {
        let input_numbers_ref = input_numbers.clone();
        threads.push(thread::spawn(|| {
            let number_opt = get_a_number(input_numbers_ref);
