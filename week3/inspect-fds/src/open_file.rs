...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        // 1. use read_link to read the destination that /proc/{pid}/fd/{fd} points to
        let link_path = format!("/proc/{}/fd/{}", pid, fd);
        let path_buf = fs::read_link(link_path).ok()?;
        let path_name = OpenFile::path_to_name(path_buf.to_str().unwrap());

        // 2. use read_to_string to read the content of /proc/{pid}/fdinfo/{fd}
        let content_path = format!("/proc/{}/fdinfo/{}", pid, fd);
        let content = fs::read_to_string(content_path).ok()?;
            // 2-1. parse_cursor
        let cursor = OpenFile::parse_cursor(content.as_str()).unwrap();
            // 2-2. parse_access_mode
        let access_mode = OpenFile::parse_access_mode(content.as_str()).unwrap();
            // 3. return a new OpenFile struct
        return Some(OpenFile::new(path_name, cursor, access_mode))
    }
