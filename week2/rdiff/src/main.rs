...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    let mut dp: Grid = Grid::new(seq1_len + 1, seq2_len + 1);
    for i in 0..seq1_len + 1 {
        dp.set(i, 0, 0);
    }

    for i in 0..seq2_len + 1 {
        dp.set(0, i, 0);
    }
...
fn main() {
    ...
    let content1 : Vec<String> = read_file_lines(filename1).unwrap();
    let content2 : Vec<String> = read_file_lines(filename2).unwrap();

    let lcs_table : Grid = lcs(&content1, &content2);
    print_diff(&lcs_table, &content1, &content2, content1.len(), content2.len());
}
