...
impl Grid {
    ...
    pub fn get(&self, row: usize, col: usize) -> Option<usize> {
        if row > self.num_rows || col > self.num_cols {
            ...
            return Some(*self.elems.get(row * self.num_cols + col).unwrap())
        }
    }
    ...
    pub fn set(&mut self, row: usize, col: usize, val: usize) -> Result<(), &'static str> {
        if row > self.num_rows || col > self.num_cols {
            // why below will failed?
            // you should go with if ... else ... , otherwise it failed.
            Err("The location is out of bounds")
            //return  Err("The location is out of bounds");
        }else {
